...
> **Provenance note (added when this file was committed).** The base build spec — the
> document these `§` numbers refer to — is **not in the repository**. Source files cite it
> throughout (`§4.3`, `§5.4`, `§7.1`, `§7.4`, `§8.3`, `§11`, `§13.1`, `§15`), but only this
> delta exists in tracked form. Where the two disagree, this file wins. Where this file is
> silent, the base spec governs and you will have to ask the owner for it. Do not infer the
> base spec from the code and treat the inference as authoritative.
